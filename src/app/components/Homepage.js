...
const InfoCard = ({ image, title, description, href, buttonText }) => (
  <div
    className="relative text-center bg-transparent p-6 sm:p-8 rounded-3xl transform transition duration-300 flex-grow w-full py-40 gap-y-10 md:border-none"
    style={{
  ...
  >
    ...
    <Link href={href}>
      <button className="relative w-[300px] h-[100px]  bg-[#735bdf] hover:bg-[#49bafg] text-center text-white text-xl sm:text-2xl font-bold p-5 sm:px-6  rounded-md shadow-button transition duration-300 hover:shadow-button-hover">
        {buttonText}
      </button>
    </Link>
  </div>
...
const Homepage = () => {
  return (
    <div className="bg-white py-10">
      <div className="flex flex-col md:flex-row md:space-x-6 justify-center items-end md:items-end py-10 px-6 sm:px-10 space-y-8 md:space-y-0">
        <InfoCard
